...
<head>
  ...
  <script>
    ...
    function getCalendarEvents(dateStr = null) {
      const calendarData = JSON.parse(localStorage.getItem("calendarEvents") || "{}");
      if (!Object.keys(calendarData).length) return "저장된 일정이 없습니다. 먼저 캘린더를 저장해주세요.";
      if (dateStr) {
    ...
    function detectIntent(tokens) {
      for (let token of tokens) {
        if (KEYWORDS.weather.some(k => token.includes(k))) return "weather";
        if (KEYWORDS.calendar.some(k => token.includes(k))) return "calendar";
        if (KEYWORDS.time.some(k => token.includes(k))) return "time";
        if (KEYWORDS.greetings.some(k => token.includes(k))) return "greeting";
        if (KEYWORDS.sleep.some(k => token.includes(k))) return "sleep";
        if (KEYWORDS.youtube.some(k => token.includes(k))) return "youtube";
        if (KEYWORDS.twitter.some(k => token.includes(k))) return "twitter";
        if (KEYWORDS.naver.some(k => token.includes(k))) return "naver";
        if (KEYWORDS.map.some(k => token.includes(k))) return "map";
        if (KEYWORDS.instagram.some(k => token.includes(k))) return "instagram";
        if (KEYWORDS.delete.some(k => token.includes(k))) return "delete";
        if (token.includes("지역")) return "changeRegion";
      }
      return "unknown";
    }
    ...
    function saveToMemory(input, response) {
      memory[input] = { response, count: (memory[input]?.count || 0) + 1 };
      localStorage.setItem("chatMemory", JSON.stringify(memory));
    }

    function getFromMemory(input) {
      return memory[input]?.response || null;
    }
    ...
    async function sendChat() {
      ...
      if (rememberedResponse) {
        showSpeechBubbleInChunks(rememberedResponse);
        inputEl.value = "";
        return;
      }
      ...
      if (lowerInput.includes("파일 저장해줘") || lowerInput.includes("캘린더 저장해줘")) {
        ...
        speakText(response);
      } else if (intent === "changeRegion" || lowerInput.startsWith("지역 ")) {
        ...
        if (newCity && regionList.includes(newCity)) {
          ...
          response = newCity ? "죄송해요, 그 지역은 지원하지 않아요." : "변경할 지역을 입력해 주세요.";
        }
        ...
        response = emotion === "positive" ? `기쁘다니 저도 덩달아 기뻐요! ${tokens.join(" ")}에 대해 더 이야기해 볼까요?`
          : emotion === "negative" ? `힘들어 보이시네요… ${tokens.join(" ")} 때문에 그런가요? 제가 위로가 될 수 있으면 좋겠어요.`
          : `${tokens.join(" ")}라니 흥미롭네요. 조금 더 설명해 주시면 제가 더 잘 이해할게요!`;
      }
    ...
    function showSpeechBubbleInChunks(text, chunkSize = 15, delay = 2000) {
    ...
    window.addEventListener("load", async () => {
      initCalendar();
      updateMap();
      await updateWeatherAndEffects(false);
    });
    ...
    function initRain() {
      ...
      rainGroup.add(new THREE.Points(rainGeometry, rainMaterial));
    }
    initRain();
    ...
    houseCloudGroup.add(createHouseCloud());
    ...
    function initCloudRain() {
      ...
      cloudRainGroup.add(new THREE.Points(geometry, material));
    }
    initCloudRain();
    ...
    function updateBubblePosition(head) {
      const bubble = document.getElementById("speech-bubble");
      const headWorldPos = new THREE.Vector3();
      head.getWorldPosition(headWorldPos);
