...
<body>
  ...
  <script>
    ...
    function createCharacter() {
      ...
      const body = new THREE.Mesh(new THREE.BoxGeometry(1, 1.5, 0.5), new THREE.MeshStandardMaterial({ color: 0x00cc66 }));
      ...
      const leftArm = new THREE.Mesh(new THREE.BoxGeometry(0.2, 1, 0.2), body.material);
      leftArm.position.set(-0.7, 0.4, 0);
      const rightArm = new THREE.Mesh(new THREE.BoxGeometry(0.2, 1, 0.2), body.material);
      ...
      group.add(body, head, leftEye, rightEye, mouth, leftBrow, rightBrow, leftArm, rightArm, leftLeg, rightLeg);
      return { group, head };
    }

    // 메인 캐릭터와 추가 캐릭터 생성
    const { group: mainCharacter, head: mainHead } = createCharacter();
    mainCharacter.position.y = -1;
    scene.add(mainCharacter);
    for (let i = 0; i < 2; i++) {
      const { group: char } = createCharacter();
      char.position.set(Math.random() * 20 - 10, -1, Math.random() * 20 - 10);
      scene.add(char);
    }
    ...
    // 고급화된 빌딩 생성 함수
    function createBuilding(width, height, depth, color, windowCountX, windowCountY) {
      ...
      for (let y = 1; y < windowCountY; y++) {
        for (let x = 0; x < windowCountX; x++) {
          const windowX = -width/2 + (x + 0.5) * (width / windowCountX);
          const windowY = -height/2 + y * (height / windowCountY);
          const frame = new THREE.Mesh(new THREE.BoxGeometry(0.5, 0.9, 0.1), frameMat);
          frame.position.set(windowX, windowY, depth/2);
          buildingGroup.add(frame);
          const window = new THREE.Mesh(new THREE.BoxGeometry(0.4, 0.8, 0.1), windowMat);
          window.position.set(windowX, windowY, depth/2 + 0.02);
      ...
      const handle = new THREE.Mesh(new THREE.SphereGeometry(0.05, 8, 8), new THREE.MeshStandardMaterial({ color: 0xFFD700 }));
      handle.position.set(0.4, -height/2 + 1, depth/2 + 0.02);
    ...
    function createHouse(width, height, depth, baseColor, roofColor) {
      ...
      const window1Frame = new THREE.Mesh(new THREE.BoxGeometry(0.9, 0.9, 0.1), frameMat);
      window1Frame.position.set(-width/4, -2 + height/2, depth/2);
      ...
      const window2Frame = new THREE.Mesh(new THREE.BoxGeometry(0.9, 0.9, 0.1), frameMat);
      window2Frame.position.set(width/4, -2 + height/2, depth/2);
      houseGroup.add(window1, window1Frame, window2, window2Frame);
      
      const doorMat = new THREE.MeshStandardMaterial({ color: 0x8B4513 });
      const door = new THREE.Mesh(new THREE.BoxGeometry(1, 1.5, 0.1), doorMat);
      door.position.set(0, -2 + height/4, depth/2 + 0.01);
      const handle = new THREE.Mesh(new THREE.SphereGeometry(0.05, 8, 8), new THREE.MeshStandardMaterial({ color: 0xFFD700 }));
      handle.position.set(0.4, -2 + height/4, depth/2 + 0.02);
      houseGroup.add(door, handle);
    ...
    for (let i = 0; i < 20; i++) {
      ...
      const windowCountX = Math.floor(width / 2);
      const windowCountY = Math.floor(height / 3);
      const building = createBuilding(width, height, depth, 0x555555, windowCountX, windowCountY);
      const col = i % 10;
    ...
    function updateHouseClouds() {
      ...
      mainHead.getWorldPosition(headWorldPos);
    ...
    function createPineTree() {
      ...
      const trunk = new THREE.Mesh(new THREE.CylinderGeometry(0.2, 0.2, 2, 16), new THREE.MeshStandardMaterial({ color: 0x8B4513 }));
      trunk.position.y = -1;
      const foliage = new THREE.Mesh(new THREE.ConeGeometry(1.5, 4, 16), new THREE.MeshStandardMaterial({ color: 0x228B22 }));
    ...
    for (let i = 0; i < 10; i++) {
      const treeType = Math.random() > 0.5 ? createTree() : createPineTree();
      treeType.position.set(-50 + i * 10, -2, -15);
      scene.add(treeType);
    }
    ...
    function animate() {
      ...
      const sunAngle = (totalMin / 1440) * Math.PI * 2;
      sun.position.set(Math.cos(sunAngle) * 50, Math.sin(sunAngle) * 50, 0);
      moon.position.set(Math.cos(sunAngle + Math.PI) * 50, Math.sin(sunAngle + Math.PI) * 50, 0);
      ...
      characterLight.position.copy(mainCharacter.position).add(new THREE.Vector3(0, 5, 0));
      characterLight.intensity = isDay ? 0 : 1;
      mainCharacter.position.y = -1;
      mainCharacter.rotation.x = 0;
      
      updateWeatherEffects();
      updateHouseClouds();
      updateLightning();
      characterStreetlight.position.set(mainCharacter.position.x + 1, -2, mainCharacter.position.z);
