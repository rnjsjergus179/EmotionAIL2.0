...
<head>
  ...
  <script>
    ...
    async function sendChat() {
      ...
      // "지도 보여줘" 키워드 처리 (구글 지도가 확대되어 표시)
      if (!response && lowerInput.indexOf("지도 보여줘") !== -1) {
        response = "지도 확대 모드로 전환합니다.";
        showSpeechBubbleInChunks(response);
        // 구글 지도 확대 (zoom=18) URL로 전환: 현재 지역 기준
        const englishCity = regionMap[currentCity] || "Seoul";
        setTimeout(() => {
          window.location.href = `https://www.google.com/maps/@?api=1&map_action=map&center=${encodeURIComponent(englishCity)}&zoom=18`;
        }, 2000);
        inputEl.value = "";
        return;
      }
      
      // "교통정보" 또는 "구글 지도 교통상황" 키워드 처리 (네이버 모바일 검색 교통정보 페이지로 전환)
      if (!response && lowerInput.indexOf("교통정보") !== -1) {
        response = "네이버 교통정보를 보여드릴게요! 잠시만 기다려 주세요.";
        showSpeechBubbleInChunks(response);
        setTimeout(() => {
          window.location.href = "https://m.search.naver.com/search.naver?query=%EA%B5%90%ED%86%B5%EC%A0%95%EB%B3%B4&sm=mtp_hty.top&where=m";
        }, 2000);
        inputEl.value = "";
        return;
      }
    ...
    async function sendChat() {
      ...
      // "지도 보여줘" 키워드 처리 : 구글 지도 확대(zoom=18) 모드로 전환
      if (!response && lowerInput.indexOf("지도 보여줘") !== -1) {
        response = "지도를 확대해서 보여드릴게요!";
        showSpeechBubbleInChunks(response);
        const englishCity = regionMap[currentCity] || "Seoul";
        setTimeout(() => {
          window.location.href = `https://www.google.com/maps/@?api=1&map_action=map&center=${encodeURIComponent(englishCity)}&zoom=18`;
      ...
      // "교통정보" 또는 "구글 지도 교통상황" 키워드 처리 : 네이버 모바일 교통정보 페이지로 전환
      if (!response && lowerInput.indexOf("교통정보") !== -1) {
        response = "네이버 교통정보를 보여드릴게요! 잠시만 기다려 주세요.";
        showSpeechBubbleInChunks(response);
        setTimeout(() => {
  ...
  <div id="tutorial-overlay">
    <div id="tutorial-content">
      ...
      <p>
        ...
        "트위터 보여주게", "트위터 틔위터검색", "트위터보여", "트위터보여줘봐" 등의 키워드가 입력되면 트위터(X) 로그인 페이지(https://x.com/login?lang=ko)로 전환됩니다.<br>
        "지도 보여줘"를 입력하면 구글 지도가 확대되어 표시되며,<br>
        ...
        또한, HUD‑6의 음성 입력 버튼을 눌러 음성 입력도 할 수 있으며, 채팅창에는 미리 설정된 자동 완성 키워드가 제시됩니다.
      </p>
  ...
  <div id="tutorial-overlay">
    <div id="tutorial-content">
      ...
      <p>
        ...
        "유튜브 보여줘", "유튜브알려줘" 등 유튜브 관련 키워드를 입력하면 페이지 전체가 유튜브로 전환됩니다.<br>
        "트위터 보여주게", "트위터 틔위터검색", "트위터보여", "트위터보여줘봐" 등의 키워드가 입력되면 트위터(X) 로그인 페이지(https://x.com/login?lang=ko)로 전환됩니다.<br>
        "지도 보여줘"를 입력하면 구글 지도 확대(zoom=18) 모드로 전환되고,<br>
        "교통정보" 또는 "구글 지도 교통상황"을 입력하면 네이버 모바일 교통정보 페이지(https://m.search.naver.com/search.naver?query=%EA%B5%90%ED%86%B5%EC%A0%95%EB%B3%B4&sm=mtp_hty.top&where=m)로 전환됩니다.<br>
        "날씨 알려줘", "일정 알려줘", "시간 알려줘" 등 다양한 질문에도 응답하며,<br>
        "잘자", "좋은꿈" 등 잘자 관련, "안녕", "안녕하세요" 등 인삿말에 따른 풍부한 응답을 제공합니다.<br>
