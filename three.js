...
// Three.js 라이브러리가 HTML에 포함되어 있는지 확인하세요.
// <script src="https://cdn.jsdelivr.net/npm/three@0.132.2/build/three.min.js"></script>

// Three.js 관련 전역 변수
let scene, camera, renderer, sun, moon, stars, fireflies, rainGroup, houseCloudGroup, cloudRainGroup, lightningLight, characterGroup, head, characterStreetlight;
...
function initThreeJS() {
  ...
  sun = new THREE.Mesh(new THREE.SphereGeometry(1.5, 64, 64), sunMaterial);
  scene.add(sun);
  ...
  moon = new THREE.Mesh(new THREE.SphereGeometry(1.2, 64, 64), moonMaterial);
  scene.add(moon);
  ...
  characterStreetlight = createStreetlight();
...
function animate() {
  ...
  try {
    if (head && head.getWorldPosition) {
      head.getWorldPosition(headWorldPos);
    }
  } catch (err) {
    console.error("애니메이션 중 head.getWorldPosition 오류:", err);
  }
  ...
  const characterLight = scene.children.find(child => child instanceof THREE.PointLight && child.color.getHex() === 0xffee88);
  if (characterLight) {
...
function updateWeatherEffects(currentWeather) {
  if (!currentWeather) {
    console.warn("날씨 데이터가 제공되지 않았습니다.");
    return;
  }
...
function updateHouseClouds() {
  if (!head || !head.getWorldPosition) {
    console.warn("head 객체가 제대로 초기화되지 않았습니다.");
    return;
  }
  const headWorldPos = new THREE.Vector3();
  try {
    ...
    console.error("updateHouseClouds 오류:", err);
    return;
  }
...
function updateBubblePosition() {
  ...
  if (!bubble) return;
  if (!head || !head.getWorldPosition) {
    console.warn("head 객체가 말풍선 위치 조정에 사용 불가능합니다.");
    return;
  }
  const headWorldPos = new THREE.Vector3();
  try {
    ...
    console.error("updateBubblePosition 오류:", err);
    return;
  }
...
// 모듈로 내보내기
export { startThreeJS, updateWeatherEffects };
