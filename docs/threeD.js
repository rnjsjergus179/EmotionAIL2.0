...
import { currentWeather } from './memory.js';
...
export function animate(updateBubblePosition) {
  requestAnimationFrame(() => animate(updateBubblePosition));
  const now = new Date();
  const headWorldPos = new THREE.Vector3();
  try {
  ...
  updateHouseClouds();
  updateLightning(currentWeather);
